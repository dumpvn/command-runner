...
/**
 * Returns the terminal name derived from the active editor's file: the basename without extension.
 * Untitled or extensionless files return undefined so the caller can fall back to defaults.
 */
function terminalNameFromActiveFile(document: vscode.TextDocument): string | undefined {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('command-runner.runInTerminal', async ({ terminal }: CommandOptions = {}) => {
            ...
            if (activeEditor) {
                ...
                // Terminal name = active file's basename (without extension).
                const detectedTerminal = terminalNameFromActiveFile(activeEditor.document);
                if (detectedTerminal) {
